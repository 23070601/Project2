...
### Cách 2: Sử dụng package `serve`
Mở một cửa sổ Terminal mới và chạy:
```cmd
cd frontend
npx serve -l 5500
```
Sau đó truy cập: `http://localhost:5500/users/Login.html`
...
### ❓ Lỗi `npm : File ... npm.ps1 cannot be loaded because running scripts is disabled`
* **Nguyên nhân:** PowerShell trên Windows chặn chạy script.
* **Khắc phục:** Mở dropdown mũi tên `v` bên cạnh dấu `+` ở góc trên Terminal VS Code ➔ Chuyển từ **PowerShell** sang **Command Prompt (cmd)**.

### ❓ Lỗi `Failed to fetch` khi bấm Đăng nhập
* **Nguyên nhân 1:** Backend server chưa được khởi chạy (Chưa chạy `npm run dev`).
* **Nguyên nhân 2:** Trình duyệt chạy ở cổng khác 5500 (ví dụ 5501).
* **Khắc phục:** Đảm bảo file `backend/.env` có cấu hình `CORS_ORIGIN=*`, sau đó tắt Backend (`Ctrl + C`) và khởi chạy lại bằng `npm run dev`.

### ❓ Lỗi `Invalid email or password`
* **Khắc phục:** Khởi chạy lại Backend Server (`npm run dev`), logic trong `server.js` sẽ tự động mã hóa lại toàn bộ mật khẩu tài khoản mẫu về **`123456`**.
